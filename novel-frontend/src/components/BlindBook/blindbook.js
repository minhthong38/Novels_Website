...
const BlindBook = () => {
  ...
  const [displayedNovels, setDisplayedNovels] = useState(novels.slice(0, 7)); // Limit to 7 books
  ...
  const handleSpin = () => {
    setSpinning(true);
    setTimeout(() => {
      const randomIndex = Math.floor(Math.random() * 7); // Adjust for 7 books
  ...
  const handleChangeNovels = () => {
    const shuffledNovels = novels.sort(() => 0.5 - Math.random());
    setDisplayedNovels(shuffledNovels.slice(0, 7)); // Limit to 7 books
  };

  const handleRead = () => {
    if (selectedNovel) {
      navigate(`/novelDetail/${selectedNovel.NovelID}`);
    }
  };
      ...
      {selectedNovel && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-lg text-center">
            ...
            <img src={selectedNovel.ImageUrl} alt={selectedNovel.Title} className="w-32 h-48 mt-4 mx-auto" />
            <button
              onClick={handleRead}
        ...
        {activeTab === 'books' && (
          <ul className="list-disc pl-5">
            {displayedNovels.map((novel) => (
              <li key={novel.NovelID} className="mb-2 flex items-center">
                <img src={novel.ImageUrl} alt={novel.Title} className="w-8 h-12 mr-2" />
                {novel.Title}
              </li>
            ))}
          </ul>
